...
class multi_tflite:
	def __init__(self,ellipse_model,point_model):
		self.interpreter1 = tflite.Interpreter(model_path=ellipse_model,experimental_delegates[tflite.load_delegate('libedgetpu.so.1', {"device": "usb:0"})])
		self.interpreter2 = tflite.Interpreter(model_path=point_model,experimental_delegates[tflite.load_delegate('libedgetpu.so.1', {"device": "usb:1"})])
		...
		self.output2 = self.interpreter2.get_input_details()
	...
	def predict_img(self,frame):
		...
		output_data = self.interpreter1.get_input(self.output1[0]['index'])
